...
In general, the release version number of a Hadoop distribution consists of three parts: the version number\index{version number}, the major revision number\index{major revision number} and the minor revision number\index{minor revision number}. A Hadoop release version can be described in Figure \ref{fig:hadoop.release}.
\begin{figure}[ht]
  \centering
...
\end{figure} 
Sometimes, the revision number\index{revision number} can have a fourth part, for example \verb|0.20.203.0|, but this is relatively rare.
\subsection*{How to do it...}
Table \ref{tbl:compare.mrv1.mrv2} shows features of major Hadoop releases:
\begin{table}[h]
  \centering
  ...
  \end{tabular}
  \caption{Comparison for Hadoop MRv1 and MRv2.}\label{tbl:compare.mrv1.mrv2}
\end{table}
...
Table \ref shows configuration files for configuring a Hadoop cluster in pseudo-distributed mode:
\begin{table}[h]
  \centering
  \begin{tabular}{ll}  
    \toprule
  ...
  \end{tabular}
  \caption{Description of Hadoop configuration files for MRv1}\label{tbl:config.files.desc}
\end{table}
...
Hadoop provides a number of bash scripts for convenience of starting and stopping a cluster. Table \ref{tbl:start.stop.scripts} shows these scripts.
\begin{table}[h]
  \centering
  \begin{tabular}{p{.18\textwidth}p{.73\textwidth}}
    \toprule
    \textbf{Script} & \textbf{Description} \\ \midrule
    \textbf{start-dfs.sh} & Script to start HDFS daemons including NameNode, SecondaryNameNode and DataNode. A PID file will be created for each daemon process under default folder \${hadoop.tmp.dir}. For example, if user hduser is used to run the script, file /hadoop/tmp/hadoop-hduser-namenode.pid will be created for the NameNode daemon process. \\ 
    \textbf{stop-dfs.sh} & Script to stop HDFS daemons. This command will try to find the PIDs of the HDFS daemons and kill the processes with the PIDs. So, if the PID file is missing, this script will not work. \\
    \textbf{start-mapred.sh} & Script to start MapReduce daemons, including the JobTracker and TaskTrackers. Similar to start-hdfs.sh script, PIDs files will be created for each daemon process. \\
    \textbf{stop-mapred.sh} & Script to stop Hadoop MapReduce daemons. Similar to stop-dfs.sh script, the script will try to find the PID files and then kill those processes. \\
    \textbf{start-all.sh} & Equals to start-dfs.sh plus start-mapred.sh.\\
    \textbf{stop-all.sh} & Equals to stop-dfs.sh plus stop-mapred.sh.\\ \bottomrule
  \end{tabular}
  \caption{Description for Hadoop start/stop scripts.}\label{tbl:start.stop.scripts}
\end{table}
...
The webpage will be similar to Figure \ref{fig:jobtracker.webui}.
\begin{figure}[ht]
  \centering
...
Check the status each slave node by clicking the link, which lead us to a webpage similar to Figure \ref{fig:active.trackers}.
\begin{figure}[ht]
  \centering
  \includegraphics[width=.8\textwidth]{figs/5163os_03_03.png}
  \caption{List of active TaskTrackers}\label{fig:active.trackers}
\end{figure} 
From Figure \ref{fig:active.trackers}, we can easily check the status of the active TaskTrackers on the slave nodes. For example, we can see the count of failed tasks, the number of MapReduce slots and the heart beat seconds etc.

Check the status of slave DataNodes by opening URL master:50070. The webpage will be similar to Figure \ref{fig:namenode.webui}.
\begin{figure}[ht]
  \centering
...
By clicking the \verb|``Live Nodes''| link we can see the details of each node as shown in Figure \ref{fig:hdfs.datanodes}.
\begin{figure}[h]
  \centering
...
The job status webpage will be similar to Figure \ref{fig:mapreduce.job}.
\begin{figure}[ht]
  \centering
...
\end{figure} 
 Figure \ref{fig:mapreduce.job} tells us that the Hadoop cluster has been configured successfully!

After the teragen job finishes, we can check the node storage space usage by opening URL \url{http://master:50070/dfsnodelist.jsp?whatNodes=LIVE}. The webpage will be similar to Figure \ref{fig:hdfs.storage}.
\begin{figure}[ht]
  \centering
