...
All the nodes in a Hadoop cluster are interconnected through network devices such as switches and routers. Data can flow from one node to another over the network to finish a MapReduce job. Among the factors that can affect the performance of a Hadoop cluster, network segmentation caused by device failures is one of the most disarstrous. It can greatly degrade the cluster performance. So, highly available and resilient network architecture is one crucial design for Hadoop cluster.
