...
We will use directory ~/mnt as the mount point for ISO images. Directory ~/isoimages will be used to contain the original image files and directory ~/repo will be used as the repository folder for network installation.
...
Plugin a USB flash drive on the administrator machine and write the bootable ISO image into the USB flash drive with the following command (assuming the USB drive corresponds to /dev/sdb device file): \\
...
In this command, 1.x.y is the version of the OpenJDK to be removed, for example: 1.7.0. 
